...
public class ClassifierState {
  ...
  //// the last instance we observed during classification, meant for instrumentation
  ...
  /**
     features : (String[]) a list of full classnames for IFeatures.
     training : (Boolean) whether or not this classifier is being trained
  ...
  */
  ...
  /**
     ...
     REGRESSION : absolute error of the prediction and the label
     MULTICLASS : should be {0.0,1.0} based on whether the prediction is correct
  */
  public double loss (Object label) {
    ...
    case MULTICLASS:
	    //System.err.println(label + "\t" + categories[classifier.getCategory(classification)]);
	    return ((String)label).equals(categories[classifier.getCategory(classification)]) ?
        0.0 : 1.0;
    default :
	    return 0.0;
    }
  ...
  */
  ...
  public String serializeStateMessage (Hashtable<String,Object> stateMessage)
    throws IOException {
  ...
  public void update (Hashtable<String,Object> stateMessage)
    throws Exception {
  ...
  public SimpleImmutableEntry<String,Double>[] getDecision(double[] classification) {
    ...
    if ((form != ClassifierForm.REGRESSION) ||
        (categories.length > 1)) {
	    int c = classifier.getCategory(classification);
	    double conf = getConfidence(c, classification);
	    result[0] = new SimpleImmutableEntry(categories[c], conf);
    } else {
	    // Hardcoded confidence
	    result[0] = new SimpleImmutableEntry(""+classification[0],1.0);
    }
