...
/**
   ...
   A utility wrapper around {@code java.util.Properties}, supporting type
   ...
   for things like setting a root directory just once, and making other
   values relative to that.  Syntax is: (via example)
   <p>
   ROOT = /home/joe/project
   Data = {ROOT}/data
*/
public class JerboaProperties {
  ...
  public static double getDouble (String key, double defaultValue) throws IOException {
    ...
    if ((value = parsePropertyValue(value)) == null) {
	    logger.config("Key not fully resolvable, returning default value for " + key + " : " + defaultValue);
	    return defaultValue;
    } else
	    return Double.parseDouble(value);
  }
